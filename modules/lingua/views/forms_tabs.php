...
<?php
    ...
    echo ($locked ? "" : form_tag('language_form_tabs', url_for("{$plugin_name}/{$module_name}/forms_tabs", 'action=save')) . submit_tag(TEXT_BUTTON_SAVE) . input_hidden_tag('entities_id', $_GET['entities_id']));
?>
...
<tr>
  ...
  <td> 
    <?php
        $name = $language_tabs[$v['id']]['name'] ?? $v['name'];
  ...
  <td>
    <?php 
      $description = ($language_tabs[$v['id']]['description'] ?? $v['description']);
